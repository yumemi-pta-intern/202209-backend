...
class MessageController extends Controller
{
    ...
    public function like(Request $request, $message_uuid)
    {
        ...
        if ($like_doesnt_exist) {
            ...
            DB::transaction(function () use ($like, $message_uuid) {
                ...
                $this->update_like_count($message_uuid);
            });
        }
        return response()->json(['status' => Response::HTTP_OK]);
    }
    ...
    private function update_like_count($message_uuid)
    {
        $like_count = Like::where('message_uuid', $message_uuid)->count();
        if (in_array($like_count, [10, 20, 40, 80, 160, 320, 640, 1280, 2560, 5120, 1240])) {
            Message::find($message_uuid)->update(['like_count' => $like_count]);
        }
    }
}
